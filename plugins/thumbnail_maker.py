...
async def generate_poster_2(anime_img_url=None, custom_image_path=None, title="", genres="", synopsis="", username="", logo_url=None, crop_state=0, small_caps=False, template_url=None, color_hex="#FF6B00", offset_x=0, offset_y=0, zoom_scale=1.0):
    ...
    if color_hex: base_template = colorize_template_2(base_template, color_hex).convert('RGBA')
...
async def generate_poster_3(anime_img_url=None, custom_image_path=None, title="", genres="", synopsis="", username="", logo_url=None, small_caps=False, offset_x=0, offset_y=0, zoom_scale=1.0, imdb_data=None, custom_ep1_path=None, custom_ep2_path=None):
    ...
    # SECURE MASKING: Only mask pure white, protect UI elements
    ...
    region_mask[156:696, 201:1005] = True   # Fanart Box
    region_mask[834:975, 201:348] = True    # Ep 1
    region_mask[834:975, 1032:1182] = True  # Ep 2
    
    # Exclude Heart and Stream Now boundaries
    ...
    # Now we draw text on the already enhanced, crisp image
    temp_draw = ImageDraw.Draw(base)

    try:
        font_title = ImageFont.truetype(os.path.join(FONTS_DIR, "Roboto Black.ttf"), 75) # Increased size
        font_genres = ImageFont.truetype(os.path.join(FONTS_DIR, "Roboto Black.ttf"), 20) # Bigger & Black
        font_rating = ImageFont.truetype(os.path.join(FONTS_DIR, "Roboto Black.ttf"), 38) # Bada kiya
        font_synopsis = ImageFont.truetype(os.path.join(FONTS_DIR, "Roboto-Regular.ttf"), 22) # Thoda bada
        font_brand = ImageFont.truetype(os.path.join(FONTS_DIR, "Roboto Black.ttf"), 16) 
        font_ep_title = ImageFont.truetype(os.path.join(FONTS_DIR, "Roboto Black.ttf"), 26) # Bigger
        font_ep_dur = ImageFont.truetype(os.path.join(FONTS_DIR, "Roboto Black.ttf"), 20) # Bigger
    except:
        font_title = font_genres = font_rating = font_synopsis = font_brand = font_ep_title = font_ep_dur = ImageFont.load_default()

    disp_title = apply_small_caps(title) if small_caps else title
    title_words = disp_title.split() if disp_title else []
    title_lines = []
    
    # Title Logic: 2 words on top, 3 words below with '...'
    ...
    if synopsis:
        ...
        box_x = 1100 
        box_y = 635  
        box_w = 660 # Width adjust kiya taaki fit rahe
    ...
    # Placed safely inside the button
    season_x = 1640
    season_y = 740 
    ...
    ep_coords = [
        {
            "box": (201, 834, 348, 975),
            "title_pos": (485, 855), # Right and Down shifted
            "duration_pos": (540, 925) # Left and Down shifted
        },
        {
            "box": (1032, 834, 1182, 975),
            "title_pos": (1315, 855),
            "duration_pos": (1370, 925) 
        }
    ]
    ...
    for i, ep_info in enumerate(ep_coords):
        if i < len(eps):
            ...
            # Dark grey placeholder if everything fails
            if not thumb_img:
    ...
    # ================== TEXT RENDERING (After Enhancement) ==================

    if disp_title:
        title_y = 125
        for line in title_lines:
            temp_draw.text((1125, title_y), line, font=font_title, fill=(255, 255, 255, 255), anchor="la")
            title_y += 85

    for pos, text, font in genre_draw_commands:
        temp_draw.text(pos, text, font=font, fill=(255, 255, 255, 255), anchor="la")

    # Shifted Right and Down
    if rating and rating != "N/A":
        temp_draw.text((1160, 510), rating, font=font_rating, fill=(255, 255, 255, 255), anchor="la")
    if duration and duration != "N/A":
        temp_draw.text((1450, 510), duration.replace(" min", ""), font=font_rating, fill=(255, 255, 255, 255), anchor="la")

    for pos, text, font in synopsis_draw_commands:
        temp_draw.text(pos, text, font=font, fill=(180, 180, 180, 255), anchor="la")

    if season_text:
        temp_draw.text((season_x, season_y), season_text, font=font_brand, fill=(255, 255, 255, 255), anchor="mm")
    ...
    if disp_username:
        # Shifted right (260)
        temp_draw.text((260, 56), disp_username, font=font_brand, fill=(180, 180, 180, 255), anchor="lm")
